...
exports.handler = (event, context, callback) => {
    ...
    var now, hb, ns, uid, alert, alertExpires
    ...
    // now parse the submitted CAP XML
    ...
    if (ns == "urn:oasis:names:tc:emergency:cap:1.2") {
        ...
        sql = mysql.format(sql, [uid, event.xml, alertExpires, now])
        try {
    ...
    pool.query(sql,function(error,results) {
        ...
        callback(error, {"statusCode":"200", "body":moment(hb).format() + " " + now})
