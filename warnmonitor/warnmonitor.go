...
func postXML(message string) {
	...
	if err != nil {
		log.Println("(warnmonitor.postXML)", err)
	}
	...
	err = json.Unmarshal(body2, &n2)
	if err != nil {
		log.Println(resp.StatusCode, "-", string(body2))
	} else {
		log.Println(resp.StatusCode, "-", n2["body"])
	}
