...
var url = "/home/pi/warn.db"

// Uptime ...
func Uptime() {
	var db2, err = sql.Open("sqlite3", url)
	if err != nil {
		log.Println("DB2 open err:", err)
	}
	defer db2.Close()
	sqlStmt, err := db2.Prepare("UPDATE uptime SET lastActivity = CURRENT_TIMESTAMP WHERE id=1")
	if err != nil {
		log.Println("Update prepare error:", err)
	}
	_, err = sqlStmt.Exec()
	if err != nil {
		log.Println("Update uptime error:", err)
	}
}
